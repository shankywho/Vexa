...
The [`IndependentCalculationVerifier`](file:///Users/shankar/.ao/data/worktrees/vexa/vexa-7/backend/app/verification/engine.py#L18) recalculates financial variances directly from source database records without inspecting the agent's textual narrative.
...
4. **General Ledger Discrepancies:**
   Journal entry debit and credit totals are summed directly from [`JournalEntryLine`](file:///Users/shankar/.ao/data/worktrees/vexa/vexa-7/backend/app/db/models/ledger.py) rows to confirm double-entry balance.
...
The [`EvidenceCompletenessVerifier`](file:///Users/shankar/.ao/data/worktrees/vexa/vexa-7/backend/app/verification/engine.py#L150) enforces that required evidentiary categories exist in the dossier:
...
If [`CitationValidator`](file:///Users/shankar/.ao/data/worktrees/vexa/vexa-7/backend/app/investigation/citation_validator.py) flags even a single hallucinated citation (`record_id` not found in dossier), Gate 2 immediately fails:
```python
...
The [`PolicyGateVerifier`](file:///Users/shankar/.ao/data/worktrees/vexa/vexa-7/backend/app/verification/engine.py#L239) evaluates whether an issue can be autonomously resolved or must be escalated to a human controller:
...
The Investigation Agent is bounded by strict resource and time limits in [`app/config.py`](file:///Users/shankar/.ao/data/worktrees/vexa/vexa-7/backend/app/config.py):
...
1. Raises [`InvestigationCircuitBreakerTripped`](file:///Users/shankar/.ao/data/worktrees/vexa/vexa-7/backend/app/investigation/agent.py#L49) with reason `"STEP_LIMIT"` or `"TIMEOUT"`.
...
1. [`ReversalEngine`](file:///Users/shankar/.ao/data/worktrees/vexa/vexa-7/backend/app/action/reversal.py) creates a new `ReversalAction` record pointing to the original `ExceptionAction.id`.
