...
| :--- | :--- | :--- | :--- |
| **Close Controller** | [`CloseWorkflowController`](../../backend/app/close_workflow/controller.py) | Deterministic DAG Runner | Coordinates task lifecycle and monitors close readiness. |
| **Financial Analyst** | [`FinancialAnalystAgent`](../../backend/app/analyst/agent.py) | Hybrid (Deterministic Tools) | Generates executive summaries, account variances, and burn rate analyses. |
| **CFO Investigator** | [`CFOInvestigationAgent`](../../backend/app/investigation/agent.py) | LLM with Fallback | Formulates root-cause hypotheses over bounded evidence dossiers. |
| **Independent Verifier** | [`VerificationAgent`](../../backend/app/verification/agent.py) | Deterministic Rules Engine | Recalculates variances, verifies evidence, and enforces policy gates. |
| **Action Agent** | [`ActionAgent`](../../backend/app/action/agent.py) | Deterministic Service | Executes allowed adjustments, stages proposals, and logs audit entries. |
...
2. **Deterministic Pre-assembly:** The dossier is assembled by [`EvidenceDossierBuilder`](../../backend/app/investigation/dossier_builder.py) prior to invoking the LLM.
...
Every finding generated by the Investigation Agent must pass through [`CitationValidator`](../../backend/app/investigation/citation_validator.py):
...
LLMs frequently exhibit overconfidence, reporting 99% certainty on incorrect reasoning. Vexa uses empirical confidence calibration ([`ConfidenceCalibrator`](../../backend/app/investigation/calibration.py)):
