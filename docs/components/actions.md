...
Located at [`backend/app/action/`](../../backend/app/action/):
...
Vexa is strictly prohibited from disbursing funds or initiating transactions on external banking networks. Permitted action types in [`app.action.types.ActionType`](../../backend/app/action/types.py#L9) are strictly bounded to internal adjustments:
...
The [`ReversalEngine`](../../backend/app/action/reversal.py) guarantees:
...
The [`HumanCorrectionService`](../../backend/app/action/correction_service.py) records every human intervention where a human reviewer's final decision differs from the system recommendation.
