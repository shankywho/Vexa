...
Located at [`backend/app/streaming/`](../../backend/app/streaming/):
...
The [`AgentEventBus`](../../backend/app/streaming/bus.py#L22) is an in-memory asynchronous publish-subscribe engine:
...
The SSE bus interface is identical in both `LIVE` and `REPLAY` modes. When the system operates in replay mode, [`TracePlayer`](../../backend/app/demo/trace_player.py) emits stored events through the exact same SSE formatting, enabling frontend dashboards to render real-time animations with 100% demo consistency.
