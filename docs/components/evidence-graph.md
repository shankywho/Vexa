...
Located at [`backend/app/evidence_graph/`](../../backend/app/evidence_graph/):
...
The [`FinancialEvidenceGraph`](../../backend/app/evidence_graph/graph.py) maintains dual adjacency structures in memory:
...
The [`EvidenceRanker`](../../backend/app/evidence_graph/ranker.py) computes relevance scores for every node relative to an exception's root entity:
