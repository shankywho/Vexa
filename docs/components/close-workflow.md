...
Located at [`backend/app/close_workflow/`](../../backend/app/close_workflow/):
...
* **Legal Transitions Map:** Enforces allowed transitions defined in [`ALLOWED_CLOSE_RUN_TRANSITIONS`](../../backend/app/close_workflow/state_machine.py#L26).
...
* **`VARIANCE_ANALYSIS`**: Invokes [`FinancialAnalystAgent`](../../backend/app/analyst/agent.py) to identify balance sheet shifts.
* **`ACCRUAL_REVIEW`**: Reviews unbilled receipts (GRNI) and prepaid amortization.
* **`EXCEPTION_REVIEW`**: Coordinates [`CFOInvestigationAgent`](../../backend/app/investigation/agent.py) and [`VerificationEngine`](../../backend/app/verification/engine.py).
* **`FINAL_VERIFICATION`**: Validates close completeness via [`CloseReadinessEvaluator`](../../backend/app/close_workflow/readiness.py).
...
Policy rules governing the close run are defined in [`app/close_workflow/types.py`](../../backend/app/close_workflow/types.py#L12):
